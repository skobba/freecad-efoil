...
class CurvedArrayWorker:
    def __init__(self, 
                 ...
                 extract=False):
        obj.addProperty("App::PropertyLink",  "Base",     "EfoilWing",   "The object to make an array from").Base = base
        obj.addProperty("App::PropertyLinkList",  "Hullcurves",   "EfoilWing",   "Bounding curves").Hullcurves = hullcurves        
        obj.addProperty("App::PropertyLink",  "Path",     "EfoilWing",   "The wing path").Path = path
        obj.addProperty("App::PropertyVector", "Axis",    "EfoilWing",   "Direction axis").Axis = axis
        obj.addProperty("App::PropertyQuantity", "Items", "EfoilWing",   "Nr. of array items").Items = items
        obj.addProperty("App::PropertyFloatList","Positions", "EfoilWing","Positions for ribs (as floats from 0.0 to 1.0) -- overrides Items").Positions = Positions
        obj.addProperty("App::PropertyFloat", "OffsetStart","EfoilWing",  "Offset of the first part in Axis direction").OffsetStart = OffsetStart
        obj.addProperty("App::PropertyFloat", "OffsetEnd","EfoilWing",  "Offset of the last part from the end in opposite Axis direction").OffsetEnd = OffsetEnd
        obj.addProperty("App::PropertyFloat", "Twist","EfoilWing",  "Rotate around Axis in degrees").Twist = Twist
        obj.addProperty("App::PropertyBool", "Surface","EfoilWing",  "Make a surface").Surface = Surface
        obj.addProperty("App::PropertyBool", "Solid","EfoilWing",  "Make a solid").Solid = Solid
        obj.addProperty("App::PropertyEnumeration", "Distribution", "EfoilWing",  "Algorithm for distance between elements")
        obj.addProperty("App::PropertyBool", "DistributionReverse", "EfoilWing",  "Reverses direction of Distribution algorithm").DistributionReverse = DistributionReverse
    ...
    def makeRibs(self, obj):
        ...
        if (not hasattr(obj,"Positions") or len(obj.Positions) == 0):
            for x in range(0, sections):            
                if sections > 1:
                    d = Efoil.distribute(x / (sections - 1), obj.Distribution, obj.DistributionReverse)
        ...
        if (obj.Surface or obj.Solid) and obj.Items > 1:
            obj.Shape = Efoil.makeSurfaceSolid(ribs, obj.Solid)
        else:
    ...
    def makeRib(self, obj, posvec):
        bbox = Efoil.boundbox_from_intersect(obj.Hullcurves, posvec, obj.Axis, self.doScaleXYZ, False)
        if not bbox:
            return None
        ...
        return Efoil.scaleByBoundbox(obj.Base.Shape, bbox, self.doScaleXYZsum, copy=True)
    
    
    def execute(self, prop):
        if prop.Base and prop.Axis == Vector(0.0,0.0,0.0):
            prop.Axis = Efoil.getNormal(prop.Base)
            return
    ...
    def onChanged(self, fp, prop):
        proplist = ["Base", "Hullcurves", "Path", "Axis", "Items", "Positions", "OffsetStart", "OffsetEnd", "Twist", "Surface", "Solid", "Distribution", "DistributionReverse"]
...
class CurvedArrayViewProvider:
    ...
    def getIcon(self):
        return (os.path.join(Efoil.get_module_path(), "Resources", "icons", "curvedArray.svg"))
...
class EfoilWing():
    ...
    def Activated(self):
        FreeCADGui.doCommand("import Efoil")
        ...
        FreeCADGui.doCommand("Efoil.makeCurvedArray(%sPath=None, Items=4, OffsetStart=0, OffsetEnd=0, Surface=False)"%(options))
    ...
    def GetResources(self):
        return {'Pixmap'  : os.path.join(Efoil.get_module_path(), "Resources", "icons", "curvedArray.svg"),
...
FreeCADGui.addCommand('EfoilWing', EfoilWing())
